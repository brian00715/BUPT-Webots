...
#include <bupt_webots/get_uint64.h>
#include <bupt_webots/node_enable_pose_tracking.h>
#include <bupt_webots/node_get_orientation.h>
#include <bupt_webots/node_get_pose.h>
#include <bupt_webots/node_get_position.h>
#include <bupt_webots/supervisor_get_from_def.h>

std::vector<std::string> model_name;

void model_name_cb(std_msgs::String::ConstPtr& msg) {

    model_name.push_back(msg->data);
}
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber robot_name_sub = nh.subscribe<std_msgs::String>("/model_name", 1, model_name_cb);

  ros::ServiceClient supervisor_get_from_def_client;
  bupt_webots::supervisor_get_from_def supervisor_get_from_def_srv;
  supervisor_get_from_def_client =
    nh.serviceClient<bupt_webots::supervisor_get_from_def>(model_name + "/supervisor/get_from_def");
  ...
  bupt_webots::node_get_pose supervisor_node_get_pose_srv;
  supervisor_node_get_pose_client = nh.serviceClient<bupt_webots::node_get_pose>(robot_name + "/supervisor/node/get_pose");
